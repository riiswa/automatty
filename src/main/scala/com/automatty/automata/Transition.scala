...
case class Transition[A, B](s1: State, acceptedLetters: Set[Option[A]], s2: State) extends PartialFunction[(State, A, MemoryManager[A, B]), State] {
  ...
  def isDefinedAt(p: (State, A, MemoryManager[A, B])): Boolean = p match {
    case (s, a, pm@PushdownManager(_, _, memoryCheck)) => s1 == s && acceptedLetters.flatten.contains(a) && memoryCheck(pm.memory.head)
